...
4. 在仓库默认分支执行 `./codex-tally sync`。

Windows 使用 `.\codex-tally.exe sync`。程序不在仓库目录时，通过 `-repo` 指定路径。
...
Git hooks 和签名设置仍然生效。任务限时 10 分钟，同一仓库不能并发同步。定时任务必须能无交互完成认证与签名；日志可写到 `.state-codex-tally/sync.log`，长期运行需自行轮转。
