...
账号接口只支持按日查询，其日期时区和边界尚未核实。本机额度窗口使用接口返回的时长和重置时间，即 `[重置时间 − 窗口时长, 重置时间)`。本机默认选择 Pro 的 7 天或 Plus 的 5 小时额度窗口；URL 显式指定区间时保留该选择。Pro 不显示 5 小时筛选，接口未提供的窗口不显示，无法读取额度时使用最近 7 天。
...
本地页签可开启「实时模式」，默认关闭。开启后，每次请求完成后间隔 1 秒刷新本地用量；离开本地页签或隐藏页面时暂停，失败后间隔 30 秒重试。账号统计和顶部额度仍使用原有缓存，不随实时模式强制刷新。大日志解析较慢时，更新间隔会相应延长，不会叠加请求。

本机缓存持久化到 `.state-codex-tally/sessions.json`，按 Codex 目录隔离；只重读发生变化的文件，并发刷新合并处理。文件未变化时复用缓存，避免重复写盘和生成分享快照；跨月时仍更新月度快照。缓存包含用量事件和文件索引，不保存对话正文或凭证。解析版本变化时重建缓存。
